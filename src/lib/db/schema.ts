...
import type { MessageRole, MessageMetadata } from "@/types/chat";
import type {
  MCPTransportType,
  MCPServerStatus,
  ToolCallStatus,
...
import type { SwarmGraphNode } from "@/types/swarm";

// ═══════════════════════════════════════════════════════════════════════════════
// OMNI-PRIME — Drizzle ORM Schema (SQLite)
// All enum/union types are imported from src/types/ (single source of truth).
// PK generation uses the `uuid` package for Node 18 compatibility.
// ═══════════════════════════════════════════════════════════════════════════════
...
export const userProfiles = sqliteTable("user_profiles", {
  ...
  rawAnswers: text("raw_answers", { mode: "json" })
    ...
    .notNull()
    .default(sql`'{}'`),
  isOnboardingComplete: integer("is_onboarding_complete", { mode: "boolean" })
    .notNull()
    .default(false),
...
export const toolCalls = sqliteTable("tool_calls", {
  ...
  toolId: text("tool_id").references(() => mcpTools.id, {
    onDelete: "set null",
  }),
...
export const knowledgeDocsRelations = relations(
  knowledgeDocs,
  ({ one, many }) => ({
    owner: one(userProfiles, {
...
// Prefixed with "Db" to distinguish from the richer application-level interfaces
// in src/types/ which include expanded relations and computed fields.
// Usage: DbAgent is the raw DB row; Agent (from @/types) includes tools/knowledge.
// ═══════════════════════════════════════════════════════════════════════════════
...
export type DbAgentTool = typeof agentTools.$inferSelect;
...
export type DbToolCall = typeof toolCalls.$inferSelect;
export type NewToolCall = typeof toolCalls.$inferInsert;

export type DbKnowledgeDoc = typeof knowledgeDocs.$inferSelect;
...
export type DbMCPTool = typeof mcpTools.$inferSelect;
